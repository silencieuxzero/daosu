...
双主题（现代/古代）由时间线滑块驱动——每个时间点在茶客档案 frontmatter 的 `timeline[].theme` 中配置：

- 拖动时间线到对应时间点，页面整体切换主题（配色与内容分套）
- 首帧内联脚本恢复默认档主题，无闪烁
...
站点支持黑幕（剧透遮盖）：文字默认被黑块盖住，鼠标移上去才显示。用法是在 Markdown 里直接写：

```html
<span class="spoiler">被遮盖的文字</span>
```

试一下——这句台词被黑幕盖住了：<span class="spoiler">我是鱼，或许是鱼。</span>

试试用键盘 Tab 键聚焦它，或者用手指按住它，也会显示。
